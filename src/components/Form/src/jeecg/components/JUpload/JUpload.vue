...
import { callbackify } from 'util';
  ...
  const props = defineProps({
    ...
    // ilife: 指定响应api接口
    api: propTypes.string.def(''), //ilife: 指定上传URL
    callback: propTypes.func, //ilife: 上传回调接口
  });
  ...
  function onFileChange(info) {
    ...
    //ilife: 添加自定义回调
    if(props.callback && info.file.status === 'done'){ //仅在上传结束时触发
      //console.log("trigger JUpload customized callback", info);
      props.callback(info);
    }
  }

...
export function createPermissionGuard(router: Router) {
  ...
  router.beforeEach(async (to, from, next) => {
    ...
    /**
    //qchzhu: 开启sessionStorage后导致token无法获取，已经调整为通过参数传递，需要修改sidebar菜单栏配置
    let stateInfo = query.state.split("__");
    ...
    console.log("process state info. save corp id and agent id to session storage.");
    //** */

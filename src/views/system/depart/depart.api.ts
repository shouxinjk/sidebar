 ...
 * 同步企业微信部门及用户
 */
export const syncWeworkDepart = () => { 
  console.log("try sync department.");
  axios
    .post( glob.domainUrl+"/wework/cptp/department/"+SUITE_ID,{
      tenantId: getTenantId(),
    },{})
    .then(res => { 
      console.log("wework department sync done.",res);
      //window.location.href = window.location.href ;//刷新页面
    })
    .catch(function (error) { 
      console.log("failed sync wework department.",error);
    });
}
/**
 * 获取权限树列表

...
<script setup lang="ts">
  ...
  function doOAuth2Login() {
    if (env.value.thirdApp) {
      if (env.value.wework && route.query.code ) { // 企微服务商服务商模式：带有code及state，获取用户信息并完成登录。适用于侧边栏菜单，配置URL为构建好的oauth链接
      ...
      }else if (route.query.oauth2LoginToken) { // 判断是否携带了Token，是就说明登录成功
